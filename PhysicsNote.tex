...
\subsubsection{机械波的概念}
    ...
    波前指的是波在传播过程中位于最前面的那个波面。\vspace{5pt}
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[>=stealth,scale=0.7]
                \draw[smooth,thick,samples=100,domain=-2*pi:1*pi] plot(\x,{0.25*sin(4.0*\x r)});
                \draw[->,thick](1*pi,0)--(2*pi,0);
                \draw[smooth,thick,samples=100,domain=-2*pi:1*pi] plot(\x,{1+0.25*sin(4.0*\x r)});
                \draw[->,thick](1*pi,1)--(2*pi,1);
                \draw[smooth,thick,samples=100,domain=-2*pi:1*pi] plot(\x,{2+0.25*sin(4.0*\x r)});
                \draw[->,thick](1*pi,2)--(2*pi,2);
                \draw[smooth,thick,samples=100,domain=-2*pi:1*pi] plot(\x,{3+0.25*sin(4.0*\x r)});
                \draw[->,thick](1*pi,3)--(2*pi,3);
                \node at(2*pi+1,0) {\small 波线};
                \node at(2*pi+1,1) {\small 波线};
                \node at(2*pi+1,2) {\small 波线};
                \node at(2*pi+1,3) {\small 波线};

                \draw[dashed](-1.625*pi,-1)--(-1.625*pi,+4);
                \draw[dashed](-1.250*pi,-1)--(-1.250*pi,+4);
                \draw[dashed](-0.875*pi,-1)--(-0.875*pi,+4);
                \draw[dashed](-0.500*pi,-1)--(-0.500*pi,+4);
                \draw[dashed](-0.125*pi,-1)--(-0.125*pi,+4);
                \draw[dashed](+0.250*pi,-1)--(+0.250*pi,+4);
                \draw[dashed](+0.625*pi,-1)--(+0.625*pi,+4);
                \draw(+1.00*pi,-1)--(+1.00*pi,+4);

                \node at(-1.625*pi,-1.5) {\small 波面};
                \node at(-1.250*pi,-1.5) {\small 波面};
                \node at(-0.875*pi,-1.5) {\small 波面};
                \node at(-0.500*pi,-1.5) {\small 波面};
                \node at(-0.125*pi,-1.5) {\small 波面};
                \node at(+0.250*pi,-1.5) {\small 波面};
                \node at(+0.625*pi,-1.5) {\small 波面};
                \node at(+1.000*pi,-1.5) {\small 波前};
            \end{tikzpicture}
            \caption{波线~波面~波前}
        \end{center}
    \end{figure}
...
\subsubsection{惠更斯原理}
    惠更斯原理：介质中波动传播到的各点都可以作为发射子波的波源，在其后的任意时刻，这些子波所构成的包络就是新的波前。\\[3mm]
    惠更斯原理对于任何波动过程都是普遍适用的，只要知道某一时刻的波前位置，就可以得知任意时刻的波前位置，从而确定波的传播方向。\\[3mm]
    惠更斯原理可以解释波的直线传播：
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[>=stealth,scale=1.0]
                ...
                \draw(0,+2.0) arc(-90:90:0.5);
                \draw(0,+1.5) arc(-90:90:0.5);
                \draw(0,+1.0) arc(-90:90:0.5);
                \draw(0,+0.5) arc(-90:90:0.5);
                \draw(0,+0.0) arc(-90:90:0.5);
                \draw(0,-0.5) arc(-90:90:0.5);
                \draw(0,-1.0) arc(-90:90:0.5);
                \draw(0,-1.5) arc(-90:90:0.5);
                \draw(0,-2.0) arc(-90:90:0.5);
                \draw(0,-2.5) arc(-90:90:0.5);
                \draw(0,-3.0) arc(-90:90:0.5);                
            \end{tikzpicture}
            \caption{惠更斯原理}
        \end{center}
    \end{figure}\\
    如图所示，左侧是原来的波前，右侧是之后的波前，根据惠更斯原理左侧的波前上每一点都可以视为新的子波源，
    这些子波源形成的的子波所构成的包络面就是右侧的波前。\\[3mm]
    由此可见，波在均匀介质中传播的过程中，波前的几何形状是基本不变的。
...
\subsubsection{波的折射}
    波的折射：波传递到两种介质的分界面上时，波的传播方向在新的介质中发生改变。\\[3mm]
    波的入射角$\alpha$和反射角$\beta$的关系：
    \begin{large}
        \begin{equation*}
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[>=stealth,scale=1.0]
                ...
                \draw (-1.75,1)--(0,0);
                \draw[->] (0,0)--(+1.75,-0.5);
                \draw (+1.75,-0.5)--(+3.50,-1.0);

                \draw(0,0.4) arc(90:150:0.4);
                \draw(0,-0.4) arc(-90:-16:0.4);
...
\subsubsection{波的衍射}
    ...
    根据惠更斯原理，当缝隙较大时，波前的形状改变不明显，衍射现象不明显：
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[>=stealth,scale=1.0]
                \draw[->](-6,+2)--(-0.5,+2);
                ...
                \draw[thick](0,-2.0) arc(-90:0:0.5);
                \draw[thick](0,+2.0) arc(+90:0:0.5);
            \end{tikzpicture}
            \caption{缝隙较大时的衍射}
        \end{center}
    \end{figure}\\
    根据惠更斯原理，当缝隙较小时，波前的形状改变较明显，衍射现象较明显：
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[>=stealth,scale=1.0]
                \draw[->](-6,+2)--(-0.5,+2);
                ...
                \draw[thick](0,-1.0) arc(-90:0:0.5);
                \draw[thick](0,+1.0) arc(+90:0:0.5);
            \end{tikzpicture}
            \caption{缝隙较小时的衍射}
        \end{center}
    \end{figure}\\
    需要指出的是，只有障碍物和波长相近时衍射现象才会较为明显，过大或过小均均不明显。\\[5mm]
...
% \subsubsection{光的反射}
%     光的反射：光传递到两种介质的分界面上时，光的传播方向调转回到原来的介质中。\\[3mm]
%     光的入射角$\theta_1$和反射角$\theta_2$的关系：
%     \begin{large}
%         \begin{equation*}
%             \theta_1=\theta_2
%         \end{equation*}
...
% \subsubsection{光的折射}
%     光的折射：光传递到两种介质的分界面上时，光的传播方向在新的介质中发生改变。\\[3mm]
%     光的入射角$\theta_1$和反射角$\theta_2$的关系：
%     \begin{large}
%         \begin{equation*}
...
%         \end{center}
%     \end{figure}\\
%     折射公式中，符号$v_1$指的是光在上方介质中的速度，符号$v_2$指的是下方介质中的速度。
...
%                 \node at(0.19,2.13) {$\Delta$};

%                 \draw(0,0) arc(180.00:158.20:5);
%                 \draw(4.7,0) arc(180.00:158.20:0.3);;

%                 \node at(4.4,0.11) {$\theta$};
%             \end{tikzpicture}
%             \caption{光学中的小角度近似}
%         \end{center}
%     \end{figure}\\
...
% \subsubsection{费马最短时间原理}
%     费马最短时间原理：光从一点行进到另一点时，光走的是所用时间最短的路径。\\[3mm]
%     费马最短时间原理中，也可以将时间最短替换为光程最短，因为两者是等价的。

% \subsubsection*{1.费马最短时间原理对反射的解释}
%     费马最短时间原理可以很好的解释光的反射。\\[3mm]
%     考虑一束光线从点$P$出发反射至点$Q$，现通过费马最短时间原理研究光线于何点反射。\\[3mm]
%     考虑以下示意图：
%     \begin{figure}[h]
%         \begin{center}
...
%                 \draw (-3.5,2)--(0,0);
%                 \draw (+4.375,2.5)--(0,0);
%                 \draw[dashed] (0,0)--(+4.375,-2.5);
%                 \draw (-3.5,2)--(-2,0);
%                 \draw (+4.375,2.5)--(-2,0);
%                 \draw[dashed] (-2,0)--(+4.375,-2.5);
...
%                 \draw[dashed] (+4.375,+2.5)--(+4.375,-2.5);
%                 \node at(+4.675,0.3) {$H$};
...
%                 \node at(0.3,-3) {$F_2$};
%             \end{tikzpicture}
%             \caption{费马最短时间原理对反射的解释}
%         \end{center}
%     \end{figure}\\
%     过反射面作点$Q$的对称点$M$，联结$PM$交反射于点$X$，可以证明点$X$就是所求点。\\[3mm]
%     因为点$P$经点$X$至点$Q$的光程为：
%     \setcounter{equation}{0}
%     \begin{align}
%         l_{PXQ}
%         &=l_{PX}+l_{QX}\\[1mm]
...
% \subsubsection*{2.费马最短时间原理对折射的解释}
%     费马最短时间原理可以很好的解释光的折射。\\[3mm]
%     考虑一束光线从点$P$出发折射至点$Q$，现通过费马时间最短原理研究光线于何点折射。\\[3mm]
%     考虑以下示意图：
%     \begin{figure}[h]
%         \begin{center}
%             \begin{tikzpicture}[>=stealth,scale=0.95]
...
%                 \draw[dashed] (+0.5,0)--(0.675,+0.482);
%                 \draw[dashed] (+2.0,0)--(1.560,-0.683);
...
%         \end{center}
%     \end{figure}\\
%     设点$X$是折射时所经点，设点$Y$是其附近一点，可以推导出点$X$应当处于什么位置。\\[3mm]
%     若点$X$为正确的折射点，那么将其移至附近的点$Y$时，光程应当基本不变。\\[6mm]
%     过点$X$作$YQ$的垂线$XA$，由于角$\angle XQA$很小：
%     \setcounter{equation}{0}
%     \begin{align}
%         &PB=PY\\[3mm]
%         &\,n_1\cdot PB=n_1\cdot PY\\[3mm]
%         &\,l_{PB}=l_{PY}
%     \end{align}\\
%     过点$Y$作$XP$的垂线$YB$，由于角$\angle YPB$很小：
%     \begin{align}
%         &QA=QX\\[3mm]
%         &\,n_2\cdot QA=n_2\cdot QX\\[3mm]
%         &\,l_{QA}=l_{QX}
%     \end{align}
...
% \subsubsection{单折射曲面的情况}
%     我们将要研究只有一个折射曲面的情况，其左侧为介质$n_1$，其右侧为介质$n_2$。\\[3mm]
%     考虑以下示意图：
%     \begin{figure}[h]
%         \begin{center}
...
%                 \tkzDefPoint(-0.236,0){V}
                
%                 \tkzDrawSegments(O1X,O2X)
%                 \tkzDrawSegments(O1,P)
%                 \tkzDrawSegments(O2,P)
...
%                 \tkzMarkRightAngle[scale=0.4](P,Q,C)
%                 \tkzDrawArc[delta=20,color=black](C,P)(T)

%                 \tkzLabelSegments[above](O1,P){$s_1$}
%                 \tkzLabelSegments[above](O2,P){$s_2$}
%                 \tkzLabelSegments[right](P,Q){$h$}
%                 \tkzLabelSegments[pos=0.55,right=2.0pt](P,C){$r$}
...
%                 \tkzLabelPoint[below](n1){$n_1$}
%                 \tkzLabelPoint[below](n2){$n_2$}
%             \end{tikzpicture}
%             \caption{单折射曲面的情况}
%         \end{center}
%     \end{figure}\\
...
%     考虑当$s_2\rightarrow +\infty$时，此时光线从右侧平行射出。\\[3mm]
%     考虑以下示意图：\vspace{-40pt}
%     \begin{figure}[h]
%         \begin{center}
...
%                 \tkzDefPoint(-1,+0.70){A1}
%                 \tkzDefPoint(-1,+0.35){A2}
%                 \tkzDefPoint(-1,+0.00){A3}
%                 \tkzDefPoint(-1,-0.35){A4}
%                 \tkzDefPoint(-1,-0.70){A5}

%                 \tkzDrawSegments[->](O1,A1)
%                 \tkzDrawSegments[->](O1,A2)
%                 \tkzDrawSegments[->](O1,A3)
%                 \tkzDrawSegments[->](O1,A4)
%                 \tkzDrawSegments[->](O1,A5)
...
%                 \tkzDrawSegments(A1,B1)
%                 \tkzDrawSegments(A2,B2)
%                 \tkzDrawSegments(A3,B3)
%                 \tkzDrawSegments(A4,B4)
%                 \tkzDrawSegments(A5,B5)

%                 \tkzDefShiftPoint[B1](1,0){C1}
%                 \tkzDefShiftPoint[B2](1,0){C2}
%                 \tkzDefShiftPoint[B3](1,0){C3}
%                 \tkzDefShiftPoint[B4](1,0){C4}
%                 \tkzDefShiftPoint[B5](1,0){C5}

%                 \tkzDefPoint(2,+1){T1}
%                 \tkzDefPoint(2,-1){T2}

%                 \tkzInterLL(B1,C1)(T1,T2)
%                 \tkzGetPoint{D1}
%                 \tkzInterLL(B2,C2)(T1,T2)
%                 \tkzGetPoint{D2}
%                 \tkzInterLL(B3,C3)(T1,T2)
%                 \tkzGetPoint{D3}
%                 \tkzInterLL(B4,C4)(T1,T2)
%                 \tkzGetPoint{D4}
%                 \tkzInterLL(B5,C5)(T1,T2)
%                 \tkzGetPoint{D5}

%                 \tkzDrawSegments[->](B1,D1)
%                 \tkzDrawSegments[->](B2,D2)
%                 \tkzDrawSegments[->](B3,D3)
%                 \tkzDrawSegments[->](B4,D4)
%                 \tkzDrawSegments[->](B5,D5)

%                 \tkzDefPoint(5,+1){U1}
%                 \tkzDefPoint(5,-1){U2}

%                 \tkzInterLL(B1,C1)(U1,U2)
%                 \tkzGetPoint{E1}
%                 \tkzInterLL(B2,C2)(U1,U2)
%                 \tkzGetPoint{E2}
%                 \tkzInterLL(B3,C3)(U1,U2)
%                 \tkzGetPoint{E3}
%                 \tkzInterLL(B4,C4)(U1,U2)
%                 \tkzGetPoint{E4}
%                 \tkzInterLL(B5,C5)(U1,U2)
%                 \tkzGetPoint{E5}

%                 \tkzDrawSegments(D1,E1)
%                 \tkzDrawSegments(D2,E2)
%                 \tkzDrawSegments(D3,E3)
%                 \tkzDrawSegments(D4,E4)
%                 \tkzDrawSegments(D5,E5)

%                 \tkzDefPoint(-2.0,-1.2){n1};
%                 \tkzDefPoint(+2.5,-1.2){n2};

%                 \tkzLabelPoint[below](n1){$n_1$}
%                 \tkzLabelPoint[below](n2){$n_2$}
%             \end{tikzpicture}
%             \caption{当$s_2\rightarrow +\infty$的折射情况}
%         \end{center}
%     \end{figure}\\
%     在这种情况下我们定义左焦距$f_1$：
%     \setcounter{equation}{0}
...
%         f_1=s_1
%     \end{align}\\
%     即当物距等于左焦距时，光会在右侧平行射出。\\[3mm]
%     依照条件代入可以推导左焦距$f_1$：\vspace{5pt}
%     \begin{align}
%         &\frac{n_1}{s_1}+\frac{n_2}{s_2}=\frac{n_2-n_1}{r}\\[5mm]
%         &\frac{n_1}{f_1}+\lim_{s_2\rightarrow\infty}\left(\frac{n_2}{s_2}\right)=\frac{n_2-n_1}{r}\\[5mm]
%         &\frac{n_1}{f_1}=\frac{n_2-n_1}{r}\\[5mm]
%         &f_1=\frac{n_1\cdot r}{n_2-n_1}
%     \end{align}\\
%     由此我们得到左焦距的计算公式：
%     \begin{large}
%         \begin{equation*}
...
%     考虑当$s_1\rightarrow +\infty$时，此时光线从左侧平行射入。\\[3mm]
%     考虑以下示意图：
%     \begin{figure}[h]
%         \begin{center}
%             \begin{tikzpicture}[scale=1.0]
...
%                 \tkzDrawSegments(O2,A1)
%                 \tkzDrawSegments(O2,A2)
%                 \tkzDrawSegments(O2,A3)
%                 \tkzDrawSegments(O2,A4)
%                 \tkzDrawSegments(O2,A5)
...
%                 \tkzDrawSegments[->](B1,A1)
%                 \tkzDrawSegments[->](B2,A2)
%                 \tkzDrawSegments[->](B3,A3)
%                 \tkzDrawSegments[->](B4,A4)
%                 \tkzDrawSegments[->](B5,A5)

%                 \tkzDefShiftPoint[B1](-2,0){C1}
%                 \tkzDefShiftPoint[B2](-2,0){C2}
%                 \tkzDefShiftPoint[B3](-2,0){C3}
%                 \tkzDefShiftPoint[B4](-2,0){C4}
%                 \tkzDefShiftPoint[B5](-2,0){C5}
...
%                 \tkzDrawSegments[->](E1,D1)
%                 \tkzDrawSegments[->](E2,D2)
%                 \tkzDrawSegments[->](E3,D3)
%                 \tkzDrawSegments[->](E4,D4)
%                 \tkzDrawSegments[->](E5,D5)
...
%             \end{tikzpicture}
%             \vspace{-60pt}
%             \caption{当$s_1\rightarrow +\infty$的折射情况}
%         \end{center}
%     \end{figure}\\
%     在这种情况下我们定义右焦距$f_2$：
%     \setcounter{equation}{0}
...
%         f_2=s_2
%     \end{align}\\
%     即当像距等于右焦距时，光会在左侧平行射入。\\[3mm]
%     依照条件代入可以推导左焦距$f_2$：\vspace{5pt}
%     \begin{align}
%         &\frac{n_1}{s_1}+\frac{n_2}{s_2}=\frac{n_2-n_1}{r}\\[5mm]
%         &\frac{n_2}{f_2}+\lim_{s_1\rightarrow\infty}\left(\frac{n_1}{s_1}\right)=\frac{n_2-n_1}{r}\\[5mm]
%         &\frac{n_2}{f_2}=\frac{n_2-n_1}{r}\\[5mm]
...
% \subsubsection{双折射面的情况}
%     我们将要研究只有两个折射面的情况，其两侧为介质$n_1$，其中间为介质$n_2$。\\[3mm]
%     考虑以下示意图：
%     \begin{figure}[h]
%         \begin{center}
%             \begin{tikzpicture}[scale=1.2]
...
%                 \tkzMarkRightAngle[scale=0.4](P,Q,C)
%                 \tkzDrawArc[color=black](C,P)(T)
%                 \tkzDrawArc[color=black](D,P)(T)
...
%                 \tkzDefPoint(-0.802,0){T1}
%                 \tkzDefPoint(-0.272,0){T2}

%                 \tkzLabelSegments[above](T1,T2){$T$}
%                 \tkzDrawSegments[<->](T1,T2)
            
%                 \tkzDefPoint(-2.5,-0.7){n1A};
%                 \tkzDefPoint(+2.5,-0.7){n1B};
%                 \tkzDefPoint(-0.38,-0.7){n2};

%                 \tkzLabelPoint[below](n1A){$n_1$}
%                 \tkzLabelPoint[below](n1B){$n_1$}
%                 \tkzLabelPoint[below](n2){$n_2$}
%             \end{tikzpicture}
%             \caption{双折射曲面的情况}
%         \end{center}
%     \end{figure}\\
%     这种由两个折射曲面构成的光学器具称为透镜，通常由玻璃构成。\\[3mm]
%     透镜的左侧球面的半径记作$r_1$，若向左凸则为正，若向右凸则为负。\\[3mm]
%     透镜的右侧球面的半径记作$r_2$，若向左凸则为正，若向右凸则为负。\\[3mm]
%     为便于计算进行以下近似：只考虑折射曲面的间距非常小的情况，这样的透镜也被称为薄透镜。
